...
export interface CodebaseScan {
  fileTree: string[];
  fileSignatures: { file: string; signatures: string[] }[];
  truncated: boolean;
}

function walkDirectory(rootPath: string, currentPath: string, fileList: string[]): void {
  if (fileList.length >= MAX_FILES_SCANNED) return;
...
export function scanCodebase(workspaceRoot: string): CodebaseScan {
  const fileList: string[] = [];
  walkDirectory(workspaceRoot, workspaceRoot, fileList);

  const fileSignatures: { file: string; signatures: string[] }[] = [];
 ...
 * doc generation.
 */
export function formatScanForPrompt(scan: CodebaseScan): string {
